...
def plot_text(vals, what, name):
    if what=="prob":
        # plt.title("GPT-2 Generated Text")
        ourvals = vals[0]
        ...
        plt.plot(x, y, color='#05D865')
        plt.ylim(0, 60000)
        plt.savefig(name + "_rank.png")
        # plt.show()
        plt.close('all')


def main_code(raw_text, language_model):
    ...
    if 'base' in language_model:
        allPayloads = []
        lm = BERTLM(model_name_or_path='bert-base-cased')
        try:
            payload = lm.check_probabilities(raw_text, topk=1)
            allPayloads.append(payload)
        except:
            for k in range(300, len(raw_text.split()), 300):
                try:
                    currChunk = raw_text.split()[k - 300:k]
                    curr_raw_text = ''.join(str(e) + " " for e in currChunk)
                    payload = lm.check_probabilities(curr_raw_text, topk=1)
                    allPayloads.append(payload)
        ...
        allPayloads = []
        lm = BERTLM(model_name_or_path='bert-large-cased')
        try:
            payload = lm.check_probabilities(raw_text, topk=1)
            allPayloads.append(payload)
        except:
